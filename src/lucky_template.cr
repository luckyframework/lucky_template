...
module LuckyTemplate
  ...
  def write!(location : Path, & : Folder ->) : Folder
    create_parent_folder do |folder|
      yield folder
    end.tap do |folder|
  ...
  def validate!(location : Path, folder : Folder) : Bool
    snapshot_files(folder).each do |filepath, type|
